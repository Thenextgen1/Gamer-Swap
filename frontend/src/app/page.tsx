...
export default function Home() {
  return (
    <main className="">
      <section>
        <p
          className={`text-center py-3 px-2 bg-[#2c6e493d]  font-Poppins text-sm tracking-wide opacity-80`}
        >
          <span className="mr-4">
      ...
      <section className="bg-[#FAFAFA] px-4 sm:px-12">
        <div className=" mx-auto max-w-7xl pb-12 tracking-wide">
          <ul className="grid grid-cols-2 lg:grid-cols-4 ">
            {trade.map((details) => (
              <li
                className="font-Poppins text-center m-4  text-[1.5em] sm:text-[2.1em] flex-wrap font-semibold"
                key={details.id}
              >
                {details.amount}
